...
describe('objectFromTuples', () => {
    ...
    it('should return an object that matches input collection of tuples', () => {
        const fn = (x) => x;
        assert.deepStrictEqual(objectFromTuples([["/", false], ["str", 'str'], ["num", 1], ['fn', fn]]), { "/": false, str: "str", num: 1, fn });
        assert.deepStrictEqual(objectFromTuples([["num", 1], ['fn', fn], ["str", 'str']]), { num: 1, fn, str: "str" });
        assert.deepStrictEqual(objectFromTuples([["x", true]]), { x: true });
    });
});
describe('keys', () => {
...
describe('merge', () => {
    ...
    it('returns merged objects for multiple inputs', () => {
        assert.deepStrictEqual(mergeDeep()({}, {}, { foo: 'bar' }, {}, {}, { foo: 'baz' }), { foo: 'baz' });
    });
    ...
    it('does perform deep merge on multiple parameters', () => {
        assert.deepStrictEqual(mergeDeep()({ foo: { bar: 'baz' } }, { foo: { baz: 'bar' } }, { baz: 'foo' }), { foo: { bar: 'baz', baz: 'bar' }, baz: 'foo', });
    });
