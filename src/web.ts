...
function getHttpData(data: RequestData): { "Content-Type"?: string, body: unknown } {
	switch (data.type) {
		case "url":
			return {
...
const getResponseHandler = (resolve: resolveFn, reject: rejectFn, opts?: { badHttpCodeAsError: boolean }) => (
...
export async function getAsync(request: GetRequest, opts?: { badHttpCodeAsError: boolean }): Promise<Response> {
	return new Promise<Response>((resolve, reject) => {
		...
		get({
			uri: `${new stdString(request.uri).trimRight("/")}${queryParams}`,
