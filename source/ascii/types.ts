import type { Digit, DigitNonZero, DecrementNonZero } from "../numeric"
import type { Rec, TypeAssert } from "../common"
...
export type UnionOfRepeats<Str extends string | number, Max extends DigitNonZero, Sep extends string = " "> = Max extends 1
	? Str
...
export type KeysToCamelCase<T> = {
	[K in keyof T as CamelCase<string & K>]: T[K] extends Rec ? KeysToCamelCase<T[K]> : T[K]
}
...
export type KeysToDashCase<T> = {
	[K in keyof T as DashCase<string & K>]: T[K] extends Rec ? KeysToDashCase<T[K]> : T[K]
}
...
export type KeysToSnakeCase<T> = {
	[K in keyof T as SnakeCase<string & K>]: T[K] extends Rec ? KeysToSnakeCase<T[K]> : T[K]
}
...
export type KeysToInitCapCase<T> = {
	[K in keyof T as InitialOnlyCapital<string & K>]: T[K] extends Rec ? KeysToInitCapCase<T[K]> : T[K]
}
